...
public class TaskServiceImpl implements TaskService{
    ...
    @Override
    public List<PagingTaskDTO> pagingTasks(Long userId, Long taskId, Long pageSize) {
        return findSubTasksByTasks(findTasksBySearchCondition(userId, taskId, pageSize)).stream()
                ...
                .sorted(comparing(PagingTaskDTO::getId).reversed())
                .collect(toList());
    }
    ...
    private static PagingTaskDTO convertToTaskResponse(Map.Entry<Task, List<SubTask>> task) {
        return PagingTaskDTO.of(
                task.getKey().getId(),
    ...
    private static List<PagingTaskDTO.ItemResponse> convertToItemResponseList(List<SubTask> subTasks) {
        return subTasks.stream()
                .map(TaskServiceImpl::convertToItemResponse)
                .collect(toList());
    }

    private static PagingTaskDTO.ItemResponse convertToItemResponse(SubTask subTask) {
        return PagingTaskDTO.ItemResponse.of(
                subTask.getId(),
                subTask.getItem(),
